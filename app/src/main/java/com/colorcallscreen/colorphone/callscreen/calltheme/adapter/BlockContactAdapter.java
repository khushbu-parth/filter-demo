...
import com.colorcallscreen.colorphone.callscreen.calltheme.activity.ActivityContactList;
...
public class BlockContactAdapter extends RecyclerView.Adapter<BlockContactAdapter.ContactViewHolder> {
    ...
    public BlockContactAdapter(Context context, List<ContactModel> list) {
    ...
    public void onBindViewHolder(final ContactViewHolder contactViewHolder, int i) {
        ...
        boolean z = true;
        if (((ActivityContactList) this.context).isSelected) {
        ...
        if (!contactModel.isUserImagePresentChecked()) {
            new Thread(new Runnable() { // from class: com.colorcallscreen.colorphone.callscreen.calltheme.adapter.BlockContactAdapter.1
                @Override 
                public void run() {
                    Bitmap userImageForPhoneNumber = UsersImageHandler.userImageForPhoneNumber(contactModel.getDisplayNumber(), BlockContactAdapter.this.context);
                    ...
                    if (userImageForPhoneNumber != null) {
                        ...
                        new Handler(Looper.getMainLooper()).post(new Runnable() { // from class: com.colorcallscreen.colorphone.callscreen.calltheme.adapter.BlockContactAdapter.1.1
                            @Override 
                            public void run() {
        ...
        contactViewHolder.itemView.setOnClickListener(new View.OnClickListener() { // from class: com.colorcallscreen.colorphone.callscreen.calltheme.adapter.BlockContactAdapter.2
            @Override 
            public void onClick(View view) {
                if (!((ActivityContactList) BlockContactAdapter.this.context).isSelected) {
                    if (BlockContactAdapter.this.listener != null) {
                        BlockContactAdapter.this.listener.onContactUserImageClicked(contactModel, contactViewHolder.circleImageView);
                        return;
                    }
                    return;
                }
                BlockContactAdapter blockContactAdapter = BlockContactAdapter.this;
                blockContactAdapter.selectItem(blockContactAdapter.context, contactViewHolder, contactModel);
            }
        });
        contactViewHolder.itemView.setOnLongClickListener(new View.OnLongClickListener() { // from class: com.colorcallscreen.colorphone.callscreen.calltheme.adapter.BlockContactAdapter.3
            @Override // android.view.View.OnLongClickListener
            public boolean onLongClick(View view) {
                BlockContactAdapter blockContactAdapter = BlockContactAdapter.this;
                blockContactAdapter.selectItem(blockContactAdapter.context, contactViewHolder, contactModel);
                if (!((ActivityContactList) BlockContactAdapter.this.context).isSelected) {
                    ((ActivityContactList) BlockContactAdapter.this.context).isSelected = true;
                    BlockContactAdapter.this.notifyDataSetChanged();
                }
                return true;
            }
        });
    ...
    public void selectItem(Context context, ContactViewHolder contactViewHolder, ContactModel contactModel) {
        ...
        ((ActivityContactList) context).onContactSelected(this.selectedContact);
    }

    @Override 
    public ContactViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        return new ContactViewHolder(LayoutInflater.from(this.context).inflate(R.layout.item_contacts_block, viewGroup, false));
    }
}
